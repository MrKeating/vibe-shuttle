...
const features = [{
  icon: GitMerge,
  title: "Smart Merge",
  description: "Intelligently combine files from two repos with conflict detection"
}, {
  icon: Eye,
  title: "Diff Preview",
  description: "See side-by-side comparison of conflicting files before merging"
}, {
  icon: FolderSync,
  title: "Flexible Output",
  description: "Create a new repo or push merged code to an existing one"
}, {
  icon: CheckCircle2,
  title: "Conflict Resolution",
  description: "Choose which version to keep for each conflicting file"
}];
const steps = [{
  number: "1",
  ...
  number: "2",
  title: "Select Two Repos",
  description: "Pick the source and target repositories you want to merge"
}, {
  number: "3",
  title: "Review & Resolve",
  description: "Preview file differences and resolve any conflicts"
}, {
  number: "4",
  title: "Merge & Deploy",
  description: "Create a new merged repo or push to an existing one"
}];
const Index = () => {
  const {
    isAuthenticated
  ...
  return <div className="min-h-screen relative">
      ...
      <header className="relative z-10 container mx-auto px-4 py-6">
        <nav className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {isAuthenticated ? <Button variant="glow" size="sm" onClick={() => navigate("/dashboard")}>
                <GitMerge className="w-4 h-4 mr-2" />
                Dashboard
              </Button> : <Button variant="glow" size="sm" onClick={() => navigate("/auth")}>
 ...
 <span className="gradient-text">One Click</span>
          ...
          <p className="text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto mb-8">
            Built something in Lovable and the same thing in Google AI Studio? Merge both GitHub repos with smart conflict resolution.
          </p>
        ...
        {/* How it Works */}
        <section className="max-w-5xl mx-auto mb-20 animate-fade-in" style={{
        animationDelay: '0.3s'
      }}>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {steps.map((step, index) => <div key={step.number} className="glass p-6 rounded-xl relative" style={{
        ...
        {/* Use Case */}
        <section className="max-w-4xl mx-auto mb-20 animate-fade-in" style={{
        animationDelay: '0.5s'
      }}>
            ...
            <div className="flex flex-col md:flex-row items-center gap-8">
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-2">
                  {["Lovable", "Google AI Studio", "Bolt.new", "Cursor", "Replit", "v0"].map(tool => <span key={tool} className="px-3 py-1 rounded-full bg-primary/10 text-primary text-sm">
                      {tool}
                    </span>)}
                </div>
              </div>
        ...
        {/* Features Grid */}
        <section className="max-w-5xl mx-auto animate-fade-in" style={{
        animationDelay: '0.6s'
      }}>
          <h2 className="font-heading text-2xl md:text-3xl font-bold text-center mb-12">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {features.map((feature, index) => <div key={feature.title} className="glass p-6 rounded-xl transition-all duration-300 hover:scale-[1.02] hover:border-primary/30" style={{
